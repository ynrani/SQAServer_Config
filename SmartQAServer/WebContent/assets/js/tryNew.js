...
$('.anchor').on('click',function(){
   ...
   if(width==10){
     ...
     $('#dropHere').css('width','60%');
   }else{
      $(this).parent().css('width','10px');
      $('#dropHere').css('width','calc( 80% - 10px)');
   }
});
...
$('.uu-i').droppable({
  ...
  drop: function(event,ui){
     var item = ui.draggable.closest('.uu-i');
    ...
    // containment: "#dropHere",
     drag: function( event, ui ) {
  	 ...
  	 stop: function(event,ui) {
        if(!ui.helper.closest('.uu-i').data('line')) return;
        ui.helper.css({top:-2,left:-2});
        ui.helper.closest('.uu-i').data('line').remove();
        ui.helper.closest('.uu-i').data('line',null);
        console.log('stopped');
      }
});
...
$('.con_anchor').on('mousedown',function(e){
    var cur_ui_item = $(this).closest('.uu-i');
...
$(function(){  
	...
	$("#dropHere").droppable({
	       drop: function(e, ui){
	               if(ui.draggable.hasClass("ui-item")) {
	     $(this).append($(ui.helper).clone());
	   //Pointing to the ui-item class in canvas and add new class.
	         $("#dropHere .ui-item").addClass("item-"+counts[0]);
	         //   $("#dropHere .ui-item").addClass("imgSize-"+counts[0]);
	                
	         //Remove the current class (ui-draggable and ui-item)
	         $("#dropHere .item-"+counts[0]).removeClass("ui-item ui-draggable ui-draggable-dragging ui-resizable ui-resizable-autohide");
